...
def create_completed_document(**overrides: Any) -> MockDocument:
    ...
    return create_document(
        status=DocStatus.ON_CHAIN, transaction_hash="0xcompleted_tx_123", **overrides
    )
...
def create_failed_document(**overrides: Any) -> MockDocument:
    ...
    return create_document(
        status=DocStatus.ERROR, error_message="Mock processing error", **overrides
    )


def create_document_repository(
    documents: Optional[Dict[int, MockDocument]] = None
...
def get_test_document_sets() -> Dict[str, Dict[int, MockDocument]]:
    ...
    return {
        ...
        "mixed_statuses": {
            i: doc for i, doc in enumerate(get_standard_test_documents().values(), 1)
        },
        "all_completed": {
